...
class BHDWallet extends Base {
    ...
    try {
      ...
      await Promise.all(this.walletUrls.map(async walletUrl => {
        ...
        if (this.maxPledge !== undefined && this.pledgeTo) {
          const pledgesToRemove = await this.getRemovablePledges(walletUrl, address, this.maxPledge);
          if (pledgesToRemove.length > 0) {
            await this.cancelPledges(pledgesToRemove, walletUrl, address);
        ...
        if (this.multiplesOf) {
          toPledge = parseFloat((Math.floor(toPledge / this.multiplesOf) * this.multiplesOf).toFixed(8));
        }
  ...
  async getRemovablePledges(walletUrl, address, maxPledgeAmount) {
    const pledges = await this.getPledges(walletUrl, address);

    const pledgesToRemove = [];
    let pledgeSum = 0;
    for (let pledge of pledges) {
      pledgeSum += pledge.amount;
      if (pledgeSum <= maxPledgeAmount) {
        continue;
      }
      pledgesToRemove.push(pledge);
    }

    return pledgesToRemove;
  }
